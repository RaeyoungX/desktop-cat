...
export function useCatAnimation({ bubbleRef, canvasRef }: UseCatAnimationOptions) {
  useEffect(() => {
    ...
    function setState(next: CatState) {
      ...
      if ((next === "sit" || next === "sleep" || next === "loaf") && Math.random() < 0.05) {
        setTimeout(() => {
          if (!cancelled) showBubble(IDLE_LINES[Math.floor(Math.random() * IDLE_LINES.length)]);
        }, 800);
      }
    }
    ...
    function drawSprite(spriteData: SpriteData, frame: number) {
      ...
      const scale = Math.min(CAT_CANVAS_WIDTH / srcW, CAT_SPRITE_HEIGHT / crop.cropH);
      const dw = srcW * scale;
      ...
      ctx.drawImage(
        ...
        srcW,
        crop.cropH,
        (CAT_CANVAS_WIDTH - dw) / 2,
        CAT_CANVAS_HEIGHT - dh,
        dw,
        dh,
      );
    }
    ...
    function drawState(drawStateName: CatState, alpha: number) {
      const cfg = CAT_SPRITE_CONFIGS[drawStateName];
      const spriteData = sprites[drawStateName];
      ctx.save();
      ctx.globalAlpha = alpha;
      if (dir === -1) {
        ctx.translate(CAT_CANVAS_WIDTH, 0);
        ctx.scale(-1, 1);
      }
      if (cfg && spriteData) {
        ...
        if (img && img.complete && img.naturalWidth > 0) {
          ctx.drawImage(img, 0, CAT_CANVAS_HEIGHT - CAT_SPRITE_HEIGHT, CAT_CANVAS_WIDTH, CAT_SPRITE_HEIGHT);
          lastDrawnDH = CAT_SPRITE_HEIGHT;
        }
      }
      ctx.restore();
    }
    ...
    function drawAccessories() {
      ...
      const headX = CAT_CANVAS_WIDTH / 2;
      const spriteTop = CAT_SPRITE_HEIGHT - lastDrawnDH;
    ...
    function updateStarParticles() {
      ...
      if (Math.random() < 0.04) {
        starParticles.push({
          x: CAT_CANVAS_WIDTH / 2 + (Math.random() - 0.5) * 50,
          y: CAT_SPRITE_HEIGHT - lastDrawnDH * (0.15 + Math.random() * 0.7),
          life: 1,
    ...
    function draw(timestamp: number) {
      ctx.clearRect(0, 0, CAT_CANVAS_WIDTH, CAT_CANVAS_HEIGHT);
    ...
    function isOverCat(x: number, y: number): boolean {
      if (x < 0 || y < 0 || x >= CAT_CANVAS_WIDTH || y >= CAT_CANVAS_HEIGHT) return false;
    ...
    function tick(timestamp: number) {
      ...
      if (state === "walk") {
        ...
        if (catX >= screenW - CAT_CANVAS_WIDTH) {
          catX = screenW - CAT_CANVAS_WIDTH;
          dir = -1;
        }
        if (!alertLocked && now > stateUntil) {
          const cursorLocalX = cursorX - originX;
          const cursorScreenX = cursorLocalX - CAT_CANVAS_WIDTH / 2;
    ...
    void Promise.all([
      ...
      ...Object.entries(CAT_SPRITE_CONFIGS).map(async ([key, cfg]) => {
